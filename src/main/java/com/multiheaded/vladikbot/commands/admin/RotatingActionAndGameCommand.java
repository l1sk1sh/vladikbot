...
import static com.multiheaded.vladikbot.settings.Constants.ACTION_PLAYING;
import static com.multiheaded.vladikbot.settings.Constants.ACTION_LISTENING;
import static com.multiheaded.vladikbot.settings.Constants.ACTION_WATCHING;
    ...
    class CreateCommand extends AdminCommand {
        ...
        protected void execute(CommandEvent event) {
            try {
                ...
                if (event.getArgs().toLowerCase().startsWith(ACTION_PLAYING)) {
                    action = ACTION_PLAYING;
                } else if (event.getArgs().toLowerCase().startsWith(ACTION_LISTENING)) {
                    action = ACTION_LISTENING;
                } else if (event.getArgs().toLowerCase().startsWith(ACTION_WATCHING)) {
                    action = ACTION_WATCHING;
                } else {
                    event.replyWarning(String.format("Action word must be one of `[%1$s, %2$s, %3$s]`!",
                            ACTION_PLAYING, ACTION_LISTENING, ACTION_WATCHING));
                    return;
                }
